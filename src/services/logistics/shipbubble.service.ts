...
export class ShipbubbleService implements LogisticsProvider {
  ...
  async getShippingRates(input: GetRatesInput): Promise<NormalizedRateResponse[]> {
    ...
    try {
      ...
      console.log("[Shipbubble] Normalizing rates with raw-NGN logic v4");
      const normalized: NormalizedRateResponse[] = couriers.map((rate: any) => {
        // Shipbubble quotes in NGN. We store the post-discount amount in NGN
        // minor units (kobo, i.e. NGN × 100) and the UI formats it as Naira
        // directly — no USD round-trip, no <Price> conversion.
        ...
        const priceKobo = Math.round(ngn * 100);
        return {
